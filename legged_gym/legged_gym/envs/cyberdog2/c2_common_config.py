from legged_gym.envs.cyberdog2.legged_robot_config import LeggedRobotDiffuseLocoCfg, LeggedRobotDiffuseLocoCfgPPO


class CyberCommonCfg(LeggedRobotDiffuseLocoCfg):
    class env(LeggedRobotDiffuseLocoCfg.env):
        # Diffuse Loco LeggedRobotDiffuseLocoCfg migrate
        num_state_history = 1
        num_stacked_obs = 1
        num_env_factors = 0
        obs_base_vel = True
    ...
    class init_state(LeggedRobotDiffuseLocoCfg.init_state):
    ...
    class control(LeggedRobotDiffuseLocoCfg.control):
        # PD Drive parameters:
        control_type = 'P'
    ...
    class asset(LeggedRobotDiffuseLocoCfg.asset):
        file = '{LEGGED_GYM_ROOT_DIR}/resources/robots/cyberdog2/urdf/cyberdog2_v2.urdf'
        name = "cyber2"
        foot_name = "foot"
        self_collisions = 0  # 1 to disable, 0 to enable...bitwise filter

    class domain_rand(LeggedRobotDiffuseLocoCfg.domain_rand):
    ...
    class rewards(LeggedRobotDiffuseLocoCfg.rewards):
    ...
    class terrain(LeggedRobotDiffuseLocoCfg.terrain):
        mesh_type = "plane"
        curriculum = False
        static_friction = 0.2
...
class CyberCommonCfgPPO(LeggedRobotDiffuseLocoCfgPPO):
    use_wandb = True

    class algorithm(LeggedRobotDiffuseLocoCfgPPO.algorithm):
        entropy_coef = 0.01
        learning_rate = 6e-5
        schedule = 'fixed'
